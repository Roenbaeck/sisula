var MDDB = VARIABLES.SourceDatabase ? VARIABLES.SourceDatabase : '';
/*~
...
SELECT
    @CF_ID = CF_ID
FROM
    ${MDDB}$.metadata.lCF_Configuration
WHERE
    CF_NAM_Configuration_Name = @name;
    
IF(@CF_ID is null) 
BEGIN
    INSERT INTO ${MDDB}$.metadata.lCF_Configuration (
...
ELSE
BEGIN
    UPDATE ${MDDB}$.metadata.lCF_Configuration
    SET
